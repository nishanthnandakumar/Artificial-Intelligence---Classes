...
node_s = [None]
node_s[0] = np.array([base,start_state, 0, 0]) #creates an array of 2,3,3
...
def h1(current_node, goal):
    ...
    current_node = current_node.flatten() #converting the current node to a list
    target_node = np.copy(goal)
    target_node = target_node.flatten() #converting target node to a list
    h1 = 0
    depth = 0
    f1 = 0

    for i in range(9):
        if current_node[i] != target_node[i]: #here we are checking if the tile is matching the tile in goal node
            if current_node[i] != 0: #we are ignoring the zero tile
                h1 += 1 #We are calculating heuristics
    return h1
...
def successor(node, level): #Each node is a list of of [x,y,z,w] type where x is the predecossor of y.
    ...
    if 0<=shift_tile[0]<=2 and 0<=shift_tile[1]<=2: #checking if it is within the boundary
        ...
        heuristics = h1(down_shift_tile[1], goal) + level + 1 #we are calculating f(x) where level is nothing but depth 
    ...
    if 0<=shift_tile[0]<=2 and 0<=shift_tile[1]<=2:
        ...
        heuristics = h1(up_shift_tile[1], goal) + level + 1
    ...
    if 0<=shift_tile[0]<=2 and 0<=shift_tile[1]<=2:
        ...
        heuristics = h1(right_shift_tile[1], goal) + level + 1
    ...
    if 0<=shift_tile[0]<=2 and 0<=shift_tile[1]<=2:
        ...
        heuristics = h1(left_shift_tile[1], goal) + level + 1
